...
def run():
    # 读取数据
    points = np.genfromtxt('./01_回归模型/data.csv', delimiter=',')
    # 设置学习率
    learningRate = 0.0001
    # 初始化参数
    initial_b = 0
    ...
    print('Running...')
    [b, w] = gradient_descent_runner(points, initial_b, initial_w, learningRate, num_iteration)
    print(f'After {num_iteration} iterations b = {b}, w = {w}, '
        + f'error = {compute_error_for_line_given_points(b, w, points)}')
